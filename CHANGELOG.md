...
- Upgraded io.sentry:sentry-spring-boot-4 from 8.50.1 to 8.54.0
...
- Fixed the error code of an exception that carries only a status. A `ResponseStatusException` with 401, 403 or 409 reported `INVALID_INPUT` while `AUTHENTICATION_FAILED`, `FORBIDDEN` and the new `CONFLICT` existed unused, so a failed login and a rejected input were indistinguishable by `type`. The mapping now lives in `ErrorCode.forStatus` and is shared with the fallback advice; unmapped 4xx report the new `CLIENT_ERROR` instead of `INVALID_INPUT`.
- Fixed a status declared via `@ResponseStatus` on an exception class becoming 500. `ExceptionHandlerExceptionResolver` runs before `ResponseStatusExceptionResolver`, so the catch-all matched first and the annotation was never read; `FallbackExceptionHandler` now reads it, including from a cause.
- Fixed `ResponseStatusException` without a reason producing `detail: "404 NOT_FOUND"` — the status prefix of `getMessage()`. Such a response now carries the configured generic detail.
- Fixed `fieldErrors` being gated on `include-message`; they are binding errors and now follow `include-binding-errors`, which was previously read by nothing.
- Errors raised in the security filter chain are `problem+json` too. `JwtAuthenticationFailureHandler` and `JwtTokenAuthenticationFilter` wrote via `sendError`, which dispatches to Boot's `/error` and produces a different body — this affected a missing or expired token, the most common client-facing error.
- `InternalAuthenticationServiceException` (authentication backend unreachable) no longer passes as an unlogged 401; it yields 500 and is logged at ERROR.
- `GlobalExceptionHandler.handleExceptionInternal` no longer bypasses `createResponseEntity`, skips a committed response or drops `MessageSource` resolution of the framework's problem details.
- 4xx responses are logged at DEBUG and access denials at INFO; previously only 5xx left a trace.
...
- Made all mail templates (`ResetTokenMessage`, `NewUserMessage`, `NewLoginMessage`, `ContactMessage`, `Header`, `Footer`) null-safe by adding FreeMarker default operators (`!`) to every `${…}` interpolation. Previously a missing/null branding value threw a `freemarker.core.InvalidReferenceException`, which — because mail delivery is `@Async` and the send methods catch `TemplateException` — was only logged and swallowed, so the mail silently never arrived. This notably affected the reset-token and new-user mails, whose templates were the only ones dereferencing `${mailBranding.textColor}`; the login mail rendered fine because it never touches that field. Colors fall back to sensible defaults (`#00b5d6` / `#ffffff`) to keep the inline CSS valid.
- Restricted the `/v1/api-tokens` endpoints to access tokens only (added to `ACCESS_TOKEN_ONLY_URLS`). Managing API tokens while authenticated with an API token linked the new token to the synthetic token principal (`<user>-api-token-<id>`) instead of a real user, persisting an unresolvable row that broke `GET /v1/api-tokens/all` for every admin. API tokens are now rejected on these endpoints before anything is stored, and the `ApiTokenAssembler` additionally tolerates an unresolvable `linkedUser` so a single orphaned row can no longer fail the whole listing.
- Fixed API tokens inheriting the `exp` of the user token they were created with, which made them expire within the access token lifetime instead of at their configured `validUntil`. Claim names registered by RFC 7519 (`iss`, `sub`, `aud`, `exp`, `nbf`, `iat`, `jti`) are excluded when `additionalClaims` are added to a token, so they keep the values set by the token service. `sub` was affected in the same way, so an API token carries its own username (`<user>-api-token-<id>`) instead of the creating user's e-mail address; as `sub` is used as the authenticated principal, this also changes `createdBy` and `updatedBy` for entities written through an API token and is a ⚠️ breaking change ⚠️ for downstream projects matching on those columns. See `MIGRATION.md` for details.
- Fixed `PATCH` requests being able to overwrite an entity's `id`, `updatedBy`, and `updatedAt`, in addition to the already-protected `createdBy`/`createdAt`. `AbstractEntityService.patchPreProcessing` now strips all of these fields from the request map before applying it via reflection, regardless of what the client sends. The protected field set is exposed via the new overridable `getPatchProtectedFieldNames()` so subclasses can protect additional fields. This is a ⚠️ breaking change ⚠️ for downstream projects/clients that relied on setting these fields through `PATCH`. See `MIGRATION.md` for details.
- Consolidated user field protection into that same mechanism: `AbstractUserService` now protects `source` and `passwordResetToken` via `getPatchProtectedFieldNames()`, replacing the separate `AbstractUserController.PROTECTED_USER_FIELDS` constant and its request-map filtering, which have both been removed. Since the check moved from the controller into the service, it applies to every caller of `userService.patch(...)` — notably `DefaultUserInitializer` can no longer set `source` on an existing user via `essencium.init.users[].source`. Behavior of `PATCH /v1/users/{id}` is unchanged; this is a ⚠️ breaking change ⚠️ only for subclasses referencing `PROTECTED_USER_FIELDS`. See `MIGRATION.md` for details.
...
- upgraded io.sentry:sentry-spring-boot-4 from 8.43.0 to 8.53.0 (previously io.sentry:sentry-spring-boot-starter-jakarta was renamed to io.sentry:sentry-spring-boot-4)
...
- Additional checks for API token access:
...
- handle ClassCastException for numeric type conversions in BaseEssenciumUserDetails
