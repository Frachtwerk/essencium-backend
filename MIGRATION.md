...
#### restrictions
- an `email` of one user can only be changed every 30 minutes. This value (`e-mail-update-intervall-in-minutes`) can be changed in the `application.yaml`
- the verification token has a validity of 1 month. This value (`e-mail-token-validity-in-months`) can be changed in the `application.yaml`
```yaml
app:
  ...
  security:
    ...
    e-mail-token-validity-in-months: 6
    e-mail-update-intervall-in-minutes: 1
```
