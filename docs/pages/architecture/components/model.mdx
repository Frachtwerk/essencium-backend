...
7. `role`: Represents the user's role, using a many-to-one relationship with the Role entity.
...
> **Note Paul:** Sollten wir nicht hier irgendwo dokumentieren, dass man entweder eine der Model-Libs verwenden oder die ID in der jeweiligen Anwendung selbst implemnetieren muss? Gleiches für den User: Die Libs stellen den fertig zur Verfügung, wenn man nur das Basis-Backend nimmt, muss man den noch "fertigstellen".
