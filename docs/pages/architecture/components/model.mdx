...
<Callout>
  Developers must either utilize one of the Model Libraries or implement the ID
  in their respective applications. The same goes for the User: the libraries
  provide it completely, but if one chooses only the basic back-end, they need
  to finalize the User themselves.
</Callout>
...
<Callout>
  It's important to remember that while the provided libraries offer a
  comprehensive User implementation, developers who choose the basic back-end
  will need to complete the User implementation on their own. The aim here is to
  ensure flexibility, allowing developers to select the approach that best
  aligns with their application requirements. They have the freedom to either
  leverage the pre-built libraries or custom-tailor their unique elements.
</Callout>
