...
<Callout>
  Currently, only one fixed token expiration interval is supported. However, one
  might want long-lived tokens for mobile apps in the future. We need to come up
  with a concept here.
</Callout>
