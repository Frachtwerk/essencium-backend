...
2. `UserDto`: This Data Transfer Object is responsible for carrying user-specific data between various processes. With fields like `id`, `email`, `firstName`, `lastName`, `phone`, `mobile`, `password`, `locale`, and `role`, it aids in handling data during creating and updating operations.
