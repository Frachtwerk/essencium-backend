...
To exchange a still-valid token for a fresh one, call `POST /auth/renew`.
...
This requires admin privileges. It regenerates the user's nonce, which causes all previously issued tokens to be rejected. For details on how this mechanism works, see [Security](/architecture/components/security).
