...
<Callout type="warning">
  `allowed-ip-addresses` and `trusted-proxies` must be **disjoint**. An address listed in
  `trusted-proxies` is never checked against `allowed-ip-addresses` — it is always skipped as
  infrastructure. Place only the addresses you want to authorise as clients in
  `allowed-ip-addresses`.
</Callout>
